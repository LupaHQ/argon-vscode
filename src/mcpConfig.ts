...
function updateMcpJsonFile(mcpJsonPath: string): void {
  ...
  if (fs.existsSync(mcpJsonPath)) {
    try {
      ...
      if (!existingData.mcpServers || typeof existingData.mcpServers !== 'object') {
  ...
  const lemonadeRagServer = {
    ...
    env: {
      ...
      PINECONE_API_KEY:
        "pcsk_2gFUZL_57whMAskjRznWocbNp8sFS2mKWGCn4d7rn6bvQLxSEofYtbCxrjq2k4tAF1A2Rz",
    },
  };
  ...
  try {
    ...
    logger.info(`Successfully updated LemonadeRAG MCP server in ${mcpJsonPath}`, false);
  } catch (error) {
    const errMsg = error instanceof Error ? error.message : String(error);
    logger.error(`Failed to write MCP configuration to ${mcpJsonPath}: ${errMsg}`, false, true);
  }
} 